<x-app-layout>
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="mb-4">
            ...
            <h2 class="text-3xl font-heading font-bold text-gray-900 leading-tight">Edit Folder Utama</h2>
            <p class="text-sm text-gray-500 mt-1">Sesuaikan identitas warna, ikon, dan tambah sub-kategori untuk <span class="font-bold text-gray-900">{{ $masterCategory->name }}</span>.</p>
        </div>
        ...
        <div class="bg-white rounded-[24px] p-6 sm:p-8 shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-gray-100">
            <form action="{{ route('master-categories.update', $masterCategory) }}" method="POST">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    ...
                    <div class="space-y-6">
                        <div x-data="{ color: '{{ old('color', $masterCategory->color ?? '#0ea5e9') }}' }">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" id="name" required class="w-full bg-gray-50 rounded-[14px] border-transparent focus:bg-white focus:border-primary focus:ring-primary shadow-sm text-sm h-[46px]" value="{{ old('name', $masterCategory->name) }}">
                        </div>

                        <div>
                            <label for="description" class="block font-bold text-xs tracking-wider text-gray-500 uppercase mb-2">Deskripsi (Opsional)</label>
                            <textarea name="description" id="description" rows="3" class="w-full bg-gray-50 rounded-[14px] border-transparent focus:bg-white focus:border-primary focus:ring-primary shadow-sm text-sm">{{ old('description', $masterCategory->description) }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                            ...
                            <div x-show="open" x-transition class="absolute z-50 mt-2 w-full bg-white border border-gray-100 rounded-[20px] shadow-xl overflow-hidden" style="display: none;">
                                <div class="p-3 border-b border-gray-100 bg-gray-50">
                                    <div class="relative">
                                        ...
                                        <input type="text" x-model="search" placeholder="Ketik nama ikon..." class="w-full py-2 pl-9 pr-3 text-sm bg-white border border-gray-200 rounded-[10px] focus:border-primary focus:ring-primary">
                                    </div>
                                </div>
                        ...
                        <div class="bg-gray-50/50 p-5 rounded-[20px] border border-gray-100" x-data="{ subs: [] }">
                            <label class="block font-bold text-xs tracking-wider text-gray-500 uppercase mb-3 flex items-center justify-between">
                                <span>4. Tambah Sub-Kategori Baru (Opsional)</span>
                            ...
                            <div class="space-y-3">
                                <template x-for="(sub, index) in subs" :key="index">
                                    <div class="flex gap-2">
                                        ...
                                        <button type="button" @click="subs.splice(index, 1)" class="px-3 bg-red-50 text-red-500 hover:bg-red-100 rounded-[12px] transition shadow-sm">
                                ...
                                <div x-show="subs.length === 0" class="text-[11px] text-gray-400 italic">Tekan 'Tambah Kolom' jika ingin memasukkan sub-kategori baru ke folder ini.</div>
                            </div>
                        </div>
                    </div>
                </div>
